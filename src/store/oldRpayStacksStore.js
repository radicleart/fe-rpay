...
const oldRpayStacksStore = {
  ...
  actions: {
    ...
    fetchFeeEstimate ({ commit, rootGetters }) {
      return new Promise((resolve, reject) => {
        ...
        axios.post(configuration.risidioBaseApi + '/mesh' + '/v2/accounts', data).then(response => {
          ...
          axios.get(configuration.risidioStacksApi + '/v2/fees/transfer').then((response) => {
            resolve(response.data)
          }).catch((error) => {
            if (error.response && error.response.data) {
    ...
    deployContractRisidio ({ state, dispatch, rootGetters }, data) {
      return new Promise((resolve, reject) => {
...
export default oldRpayStacksStore
